...
- Параллельная обработка пачки транзакций в разных валютах
...
- POST /batch - Принять и обработать пачку транзакций
2. Лимиты (/api/limits)
...
- Тест на параллельную обработку пачки транзакций в разных валютах
...
### Создание пачки транзакций

Метод: POST

Путь: http://localhost:8080/api/transactions/batch
```
[
    {
...
## Публичный деплой на Render.com

Сервис успешно развёрнут на бесплатном тарифе Render.com:

- Базовый URL: https://testtask-qkua.onrender.com
- Swagger UI: https://testtask-qkua.onrender.com/swagger-ui/index.html#/

## Важно

- Сервис "засыпает" через 15 минут без запросов. Просто подождите(приблизительно от 3 до 5 минут).
- База данных - Render PostgreSQL, бесплатный тариф.
- Все данные хранятся в памяти/БД до перезапуска сервиса или ручной очистки.